...
describe(
    "calcularePrice.BlueRayPackage", () => {
        test("video not selected, dont count service", () => {
            const price = calculatePrice(["WeddingSession", "BlurayPackage"], 2020);

            expect(price.finalPrice).toEqual(600);
        });

        test("video selected, count additional price", () => {
            const price = calculatePrice(["VideoRecording", "BlurayPackage"], 2020);

            expect(price.finalPrice).toEqual(2000);
        });
    }
);

describe(
    "calcularePrice.TwoDayEvents", () => {
        test("video or photography not selected, dont count service", () => {
            const price = calculatePrice(["WeddingSession", "TwoDayEvent"], 2020);

            expect(price.finalPrice).toEqual(600);
        });

        test("video selected, count additional price", () => {
            const price = calculatePrice(["VideoRecording", "TwoDayEvent"], 2020);

            expect(price.finalPrice).toEqual(2100);
        });

        test("photography selected, count additional price", () => {
            const price = calculatePrice(["Photography", "TwoDayEvent"], 2020);
